...
--   let
--     -- 1. Run folding logic *first* by faking a rule.
--     -- We don't know the ID yet, so we just use a temporary one (0)
--     -- to get the rule's content for analysis.
--     fake_active = active0 0
...
--         Just fold_fn ->
--           let
--             -- This is the equation for the rule we are *about* to add
--             current_eqn = canonicalise (unorient (active_rule fake_active))

--             -- Get all unique subterms from the new rule
--             all_subterms = usort (subterms (lhs (active_rule fake_active)) ++ subterms (rhs (active_rule fake_active)))
            
--             tryFold t = do
--               rhs <- fold_fn t
--               let new_eqn = t :=: rhs
--               -- This check prevents adding an axiom identical
--               -- to the rule we're currently adding.
--               if canonicalise new_eqn == current_eqn
--                 then Nothing 
--                 else Just new_eqn
...
--             -- Axiom numbers are just for metadata; the *rule IDs* will
--             -- be generated by the recursive 'addActive' calls.
--             new_axioms = zipWith makeAxiom [fromIntegral st_next_active..] new_fold_eqns
            
--             -- Add all new axioms to the *original* state.
--             -- This recursively calls addActive, incrementing st_next_active.
--             axiomsState = foldl' (addAxiom config) state new_axioms
            
--             -- Get the *final* st_next_active *after* all folded rules were added
--           in (axiomsState, (\(State{st_next_active = n}) -> n) axiomsState)

--     -- 2. Now, create the *real* active, using the *next available ID*
--     --    (which is 'next_active_after_axioms').
--     active = active0 next_active_after_axioms
    
--     -- 3. Add this new active to that state, and increment the counter.
--     state' =
--       message (NewActive active) $
--       addActiveOnly axiomsState{st_next_active = next_active_after_axioms + 1} active
      
--   in if subsumed (st_joinable, st_complete) st_rules (unorient (active_rule active)) then
--     axiomsState -- Original rule was subsumed, but keep folded axioms
--   else
--     -- Original rule is kept; add its CPs to the queue
      ...
      case cfg_fold_term of
        ...
        Just fold_fn ->
          let
            ...
            tryFold t = do
              rhs <- fold_fn t -- e.g., fold_fn(mul(2,3)) returns Just(6)
              let new_eqn = t :=: rhs
              
              -- VVV THIS IS THE NEW CHECK VVV
              -- If the folded equation is the same as the rule we're already
              -- adding, don't create a new axiom for it!
              if canonicalise new_eqn == current_eqn
                then Nothing 
                else Just new_eqn
            -- ^^^ END OF NEW CHECK ^^^
