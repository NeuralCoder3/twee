...
-- | Apply a substitution.
subst :: (ConstantFoldable (ConstantOf a), Symbolic a, Substitution s, SubstFun s ~ ConstantOf a) => s -> a -> a -- <<< CONSTRAINT ADDED
...
instance ConstantFoldable f => Symbolic (Subst f) where -- <<< CONSTRAINT ADDED
  type ConstantOf (Subst f) = f
  termsDL (Subst sub) = termsDL (IntMap.elems sub)
  subst_ sub (Subst s) = Subst (fmap (subst_ sub) s)

instance (ConstantOf a ~ ConstantOf b, ConstantFoldable (ConstantOf a), Symbolic a, Symbolic b) => Symbolic (a, b) where -- <<< CONSTRAINT ADDED
...
instance (ConstantOf a ~ ConstantOf b,
          ...
          ConstantFoldable (ConstantOf a), -- <<< CONSTRAINT ADDED
...
instance ConstantFoldable (ConstantOf a) => Symbolic [a] where -- <<< CONSTRAINT ADDED
  type ConstantOf [a] = ConstantOf a
  termsDL xs = msum (map termsDL xs)
  subst_ sub xs = map (subst_ sub) xs

instance ConstantFoldable (ConstantOf a) => Symbolic (Maybe a) where -- <<< CONSTRAINT ADDED
...
canonicalise :: (ConstantFoldable (ConstantOf a), Symbolic a) => a -> a -- <<< CONSTRAINT ADDED
...
renameAvoiding :: (ConstantFoldable (ConstantOf a), ConstantFoldable (ConstantOf b), Symbolic a, Symbolic b) => a -> b -> b -- <<< CONSTRAINTS ADDED
renameAvoiding x y
  | x2 < y1 || y2 < x1 =
...
renameManyAvoiding :: (ConstantFoldable (ConstantOf a), Symbolic a) => [a] -> [a] -- <<< CONSTRAINT ADDED
renameManyAvoiding [] = []
...
-- minimal constant.
erase :: (Symbolic a, ConstantOf a ~ f, Minimal f, ConstantFoldable f) => [Var] -> a -> a -- <<< CONSTRAINT ADDED
erase [] t = t
...
eraseExcept :: (Symbolic a, ConstantOf a ~ f, Minimal f, ConstantFoldable f) => [Var] -> a -> a -- <<< CONSTRAINT ADDED
eraseExcept xs t =
  erase (usort (vars t) \\ xs) t

-- | Replace all variables in the argument with the minimal constant.
ground :: (Symbolic a, ConstantOf a ~ f, Minimal f, ConstantFoldable f) => a -> a -- <<< CONSTRAINT ADDED
ground t = erase (vars t) t

-- | Skolemise the argument.
skolemise :: (Symbolic a, ConstantOf a ~ f, Minimal f, ConstantFoldable f) => a -> a -- <<< CONSTRAINT ADDED
...
type Function f = (Ordered f, Minimal f, PrettyTerm f, EqualsBonus f, Labelled f, ConstantFoldable f) -- <<< CONSTRAINT ADDED
